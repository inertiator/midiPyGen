...
import datetime, time, os, sys, random
...
class MotifBasics(object):
    ...
    def __init__(self, motifName, motifRange, motifKey):
        self.MotifName = MotifName
        self.MotifRange = MotifRange
        self.MotifKey = MotifKey
        return None
    
    #Time signature is tuple of (3,4) 4/4, etc. Beats per measure, and which gets beat
    def add_motif_lick(self,timeSignature,Genre):
        
        #Total Length of lick
        numBeats = timeSignature[0]
        lickFreq1 = np.randomint(1,timeSignature[0]+1)
        lickFreq2 = 1/float(np.randomint(1,timeSignature[0]+1))
        lickFreq = [lickFreq1,lickFreq2]
        lickFreqc = random.choice(lickFreq)
        totalLickLength = float(numBeats)/float(lickFreqc)
        
        #Subdivide into notes
        Genre.create_motif_rhythm(timeSignature)
        def create_motif_rhythm(timeSignature,totalLickLength):
