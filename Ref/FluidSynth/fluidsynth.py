...
class Synth:            # interface for the FluidSynth synthesizer
    ...
    def sfload (self, filename, update_midi_preset=0):  # load soundfont
        return F.fluid_synth_sfload (self.synth, c_char_p (filename), update_midi_preset)
...
class Player:               # interface for the FluidSynth internal midi player
    def __init__ (s, flsynth):
        ...
        s.player = F.new_fluid_player (s.flsynth.synth)

    def add (s, midifile):  # add midifile to the playlist
        F.fluid_player_add (s.player, c_char_p (midifile))
    ...
    def get_ticks (s):      # get current position in midi ticks
        t = F.fluid_player_get_ticks (s.player)
        return t
    ...
    def load (s):           # load a midi file from the playlist (to determine its length)
        F.fluid_player_load_midi (s.player)

    def get_length (s, n):  # get duration of a midi track in ticks
        return F.fluid_player_track_duration (s.player, n)

    def delete (s):
        F.delete_fluid_player (s.player)

    def renderLoop (s, quality = 0.5, callback=None):       # render midi file to audio file
        renderer = F.new_fluid_file_renderer (s.flsynth.synth)
        if not renderer:
        ...
        F.fluid_settings_getint (s.flsynth.settings, 'audio.period-size', byref (k))
        n = 0               # sample counter
    ...
    def set_render_mode (s, file_name, file_type):  # set audio file and audio type
        ...
        F.fluid_settings_setstr (st, "audio.file.name", c_char_p (file_name))
        F.fluid_settings_setstr (st, "audio.file.type", c_char_p (file_type))
